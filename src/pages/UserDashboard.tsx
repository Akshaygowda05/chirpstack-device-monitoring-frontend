    ...
    <Box sx={{ bgcolor: "background.default", minHeight: "100vh", transition: "background 0.3s ease" }}>
      <Container maxWidth="xl" sx={{ py: { xs: 3, md: 4 } }}>
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", lg: "1fr 350px" }, gap: { xs: 2, md: 2.5 }, alignItems: "start", mb: 2.5 }}>
          {/* Cleaning History */}
          <Box sx={{ bgcolor: "background.paper", borderRadius: 2, border: "1px solid", borderColor: "divider", p: { xs: 2, sm: 3 }, minHeight: 400, display: "flex", flexDirection: "column" }}>
            <Typography sx={{ fontWeight: 700, fontSize: "0.95rem", color: "text.primary", mb: 0.4 }}>Cleaning Performance</Typography>
            <Typography sx={{ fontSize: "0.76rem", color: "text.secondary", mb: 2 }}>History of panels cleaned over the last 5 days</Typography>
            <Divider sx={{ mb: 3 }} />
            <Box sx={{ flexGrow: 1, width: "100%" }}><CleaningHistoryChart /></Box>
          </Box>

          {/* Device Distribution */}
          <Box sx={{ bgcolor: "background.paper", borderRadius: 2, border: "1px solid", borderColor: "divider", p: { xs: 2, sm: 3 }, display: "flex", flexDirection: "column", alignItems: "center" }}>
            ...
            <Box sx={{ mt: 2, display: "flex", gap: 2, flexWrap: "wrap", justifyContent: "center" }}>
              {[{ label: "Active", color: "#22c55e", count: activeCount }, { label: "Offline", color: theme.palette.text.disabled, count: inactiveCount }].map((item) => (
                <Box key={item.label} sx={{ display: "flex", alignItems: "center", gap: 0.6 }}>
                  <Box sx={{ width: 10, height: 20, borderRadius: "50%", bgcolor: item.color }} />
